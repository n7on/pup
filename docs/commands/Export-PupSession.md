...
```
Export-PupSession -Page <PupPage> [-FilePath <String>] [-PassThru] [-ProgressAction <ActionPreference>]
 [<CommonParameters>]
```
...
```
Export-PupSession -Page $page -FilePath "authenticated-session.json"
```
...
Export-PupSession -Page $page -FilePath "admin-session.json"
```
